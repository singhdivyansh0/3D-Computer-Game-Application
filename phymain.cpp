...
vector fv(2.5,0.0,3.0);
vector cv(0.0,0.0,3.0);
vector bv(-2.5,0.0,3.0);
...
int* write(){
    int* tab=new int[100];
    string line;
    ifstream my("C:/ab_to_pakka/high.txt");
    if (my.is_open()){
        int i=0;
...
while(p<100){
    ...
    p++;
     ...
     ofstream myfile;
              myfile.open("C:/ab_to_pakka/final.txt");

    int i=0;
    while(i<5){
        cout<<"while first"<<endl;
        int k=1;
        int high=0;
        int max=str[0];
        cout<<max<<endl;
...
node* create(int m){
    ...
    node *ca=new node[23];
    node a(5.0,heightAt(_terrain,5.0,5.0)+.6,5.0,0.6);
    node b(175,heightAt(_terrain,175,150)+.6,150,0.6);
    node c(40,heightAt(_terrain,40,220)+.6,220,0.6);
    node d(220,heightAt(_terrain,220,50)+.6,50,0.6);
    node e(180,heightAt(_terrain,180,180)+.6,180,0.6);
            node f(60,heightAt(_terrain,60,70)+.6,70,0.6);
            node g(215,heightAt(_terrain,215,85)+.6,85,0.6);
            node h(194,heightAt(_terrain,194,20)+.6,20,0.6);
            node j(215,heightAt(_terrain,215,158)+.6,158,0.6);
            node k(90,heightAt(_terrain,90,215)+.6,215,0.6);
            node l(205,heightAt(_terrain,205,20)+.6,20,0.6);
            node m1(200,heightAt(_terrain,200,10)+6,10,6);
            node n(10,heightAt(_terrain,10,170)+6,170,6);
                        node o(30,heightAt(_terrain,30,30)+3,30,3.5);
                        node p(121,heightAt(_terrain,121,20)+3,20,3.5);
                        node q(225,heightAt(_terrain,225,211)+3,211,3.5);


                        node r(105,heightAt(_terrain,105,201)+3,201,3.5);
                                node s(110,heightAt(_terrain,110,110)+3,110,3.5);
                                node t(50,heightAt(_terrain,50,50)+2,50,2);
                                node u(140,heightAt(_terrain,140,135.0)+2,135,2);
                                node v(85,heightAt(_terrain,85,31)+2,31,2);
                                        node w(112,heightAt(_terrain,112.0,171.0)+2,171,2);
                                        node x(56,heightAt(_terrain,56,212)+2,212,2);
//											node y(30,0,30,1);
//											node z(60,0,60,1);
//											node a1(30,0,30,1);
//											node b1(60,0,60,1);
...
float angleAt(vector c){
    vector d=v.Normalise(bike.head);
    float in=0.01;
    float h=heightAt(_terrain,c.x+d.x*in,c.z+d.z*in)-heightAt(_terrain,c.x,c.z);
...
void terrain_ch(){
    ...
    if( tt ==0 ||tt==1 ||tt==2||tt==4||tt==5||tt==7||tt==8|| tt==9 ||tt==10){
    glEnable(GL_TEXTURE_2D);
        glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
        glBindTexture(GL_TEXTURE_2D, _textureId_terrain);
...
void initRendering() {
    ...
    t3dInit();
        ...
        Image* image = loadBMP("C:/ab_to_pakka/heightmap.bmp");
                        _textureId_bike= loadTexture(image);
                        delete image;

                         image = loadBMP("C:/ab_to_pakka/shu1.bmp");
                            _textureId = loadTexture(image);

                            delete image;
                            image = loadBMP("C:/ab_to_pakka/House1.bmp");
                            _textureId_house = loadTexture(image);

                            delete image;
                            image = loadBMP("C:/ab_to_pakka/treestompcolmap.bmp");
                            _textureId_treetrunk = loadTexture(image);

                            delete image;


                            image = loadBMP("C:/ab_to_pakka/txt_001_diff.bmp");
                                                        _textureId_water = loadTexture(image);

                                                        delete image;
                                                        if(tt==1 ||tt==4 ||tt==7 ||tt==10){
                                                        image = loadBMP("C:/ab_to_pakka/snow.bmp");
                                                                                    _textureId_terrain = loadTexture(image);

                                                                                    delete image;}
                                                            else if( tt ==0 || tt==2 ||tt==5 || tt==8){
                                                            image = loadBMP("C:/ab_to_pakka/sandy.bmp");
                                                                                        _textureId_terrain = loadTexture(image);
...
void accelerate(){
    ...
    if(bike.velocity.Magnitude(bike.velocity)>0.60&&bike.velocity.Magnitude(bike.velocity)<1.2){
        vector t=a.ConstProduct(n,acc);
                bike.velocity.x=(bike.velocity.x+t.x*0.04)+bike.head.x*gravity;
                bike.velocity.z=(bike.velocity.z+t.z*0.04)+bike.head.z*gravity;
        ...
        bike.center.x=(bike.center.x+bike.velocity.x+bike.head.x*bs)+bike.head.x*gravity;
        bike.center.z=(bike.center.z+bike.velocity.z+bike.head.z*bs)+bike.head.z*gravity;
    }
    else{
        bike.velocity.x=(bike.velocity.x+t.x+t.x*bs)+bike.head.x*gravity;
        bike.velocity.z=(bike.velocity.z+t.z+t.z*bs)+bike.head.z*gravity;
        bike.center.x=bike.center.x+bike.velocity.x;
        bike.center.z=bike.center.z+bike.velocity.z;
...
void hanf(){
    ...
    bike.velocity.x=bike.velocity.x+f.x+f.x*hb+bike.head.x*gravity;
    //bike.velocity.y=bike.velocity.y+f.y;
    bike.velocity.z=bike.velocity.z+f.z+f.z*hb+bike.head.z*gravity;
...
void drawNumCollisions(int numCollisions,int y,int s,float t,bool g) {
    ostringstream oss;
    ostringstream oss1;
    ostringstream oss2;
    ostringstream oss3;
    ...
    oss3  << "GAME OVER";
        ...
        oss4  << "TIME "<<120-floor(t);
               string str4 = oss4.str();


    glDisable(GL_TEXTURE_2D);
    glDisable(GL_LIGHTING);
    glColor3f(0.4f, 0.0, 0.0f);
    glPushMatrix();
...
glPushMatrix();
        ...
        drawNumCollisions(score,speed,star,Time,gameOver);
        glPopMatrix();

    float s= 2.0;
    gluLookAt((bike.center.x-bike.head.x*s+xx),bike.center.y+0.50+yy,(bike.center.z-bike.head.z*s+zz),bike.center.x,bike.center.y+0.9,bike.center.z,0.0,1.0,0.0);
    ...
    glPushMatrix();
     ...
     //   glColor3f(0.37, 0.37, 0.37);
      ...
      //  glColor3f(1,1,1) ;
        ...
        glPushMatrix();
//            glTranslatef(128,-32,128);
            glTranslatef(128,WaterLevel,128);
    ...
    int StarHeight=0.90;
                                            ...
                                            glPushMatrix();
                                              ...
                                              //glColor3f(1,1,0) ;
                                            if(arr[0]==false){
                                                glTranslatef(190.0f,_terrain->getHeight(190,40)+StarHeight, 40.0f);

                                            }
                                            else{
                                                glTranslatef(10000,_terrain->getHeight(190,40)+StarHeight, 40.0f);

                                            }
                                            glRotatef(sr,0.0,1.0,0.0);
...
if(ai==true ||bike.center.x>256||bike.center.z>256||bike.center.x<-1||bike.center.z<-1){
    ...
    if(bike.center.y-heightAt(_terrain,bike.center.x,bike.center.z)>0.150 ||bike.center.x>256||bike.center.z>256||bike.center.x<-1||bike.center.z<-1){
            ...
            if(bike.center.x>257||bike.center.z>257||bike.center.x<-2||bike.center.z<-2){
                                        if(printScore){
                                            ofstream my;
                                            my.open("C:/ab_to_pakka/high.txt",ios_base::app);
                                            my<<score<<endl;
                                            my.close();
...
int start1(int i) {
    ...
    char fakeParam[] = "fake";
      ...
      glutInit( &fakeargc, fakeargv );
        ...
        if(i==1){
            _terrain = loadTerrain("C:/ab_to_pakka/terrain256.bmp", 50);
            WaterLevel=-32.0;
            tt =1 ;

        }
        else if(i==2){
            _terrain = loadTerrain("C:/ab_to_pakka/terrain2.bmp", 50);
            tt =2 ;
             WaterLevel=-22;
        }
        else if(i==3){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain3.bmp", 50);
                tt =3 ;
                 WaterLevel= -20.7 ;

            }
        else if(i==4){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain4.bmp", 50);
                tt =4 ;
                 WaterLevel= -24 ;
            }
        else if(i==5){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain5.bmp", 50);
                tt =5 ;
                 WaterLevel= -23.2 ;
            }
        else if(i==6){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain6.bmp", 50);
                tt =6 ;
                 WaterLevel=-22 ;
            }
        else if(i==7){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain7.bmp", 50);
                tt =7 ;
                 WaterLevel=-24.5 ;
            }
        else if(i==8){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain8.bmp", 50);
                tt =8 ;
                 WaterLevel=-23 ;
            }
        else if(i==9){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain9.bmp", 50);
                tt =9 ;
                  WaterLevel=-23 ;
            }
        else if(i==10){
                _terrain = loadTerrain("C:/ab_to_pakka/terrain10.bmp", 50);
                tt =10 ;
                  WaterLevel=-22.4 ;

            }
        else if(i==0){
                        _terrain = loadTerrain("C:/ab_to_pakka/practice.bmp", 50);
                        tt =0 ;
                         WaterLevel=-21 ;
                    }
        ...
        glutCreateWindow("Dirt Bike Game");
        //_terrain = loadTerrain("C:/ab_to_pakka/terrain256.bmp", 25);
        star1 = glmReadOBJ("C:/ab_to_pakka/star.obj");

        myModel = glmReadOBJ("C:/ab_to_pakka/quad.obj");
        myModel_tree = glmReadOBJ("C:/ab_to_pakka/lowpolytree5.obj");
            myHouse = glmReadOBJ("C:/ab_to_pakka/House1.obj");
            myRock = glmReadOBJ("C:/ab_to_pakka/rock1.obj");
            treetrunk = glmReadOBJ("C:/ab_to_pakka/treestompsculpture.obj");
        initRendering();
